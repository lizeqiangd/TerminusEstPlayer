...
var player_id: string = 'TerminusEstPlayer_123123';
var player_selector: string = '#' + player_id + ' ';
var video_content: any = $(player_selector + '.video_content')[0];
// var auto_hide_controller_timer: number=0;
// let auto_hide_controller_delay: number = 3000;
var contoller_display_state: number = 2;
var screenfull: any;
...
function addUIListener() {
    ...
    $(player_selector + '.progress_bar').mousedown(
        (event)=> {
            var deltaX: number = event.pageX - $(player_selector + '.progress_bar').offset().left;
            var bar_width: number = $(player_selector + '.progress_bar').width();
    ...
    $(player_selector).mousemove(
        (event)=> {
            var deltaY: number = event.pageY - $(player_selector).offset().top;
            var judge: number = deltaY / $(player_selector).height();
            if (judge > 0.6) {
    ...
    $(player_selector).mouseout(
        (event)=> {
            var deltaY: number = event.pageY - $(player_selector).offset().top;
            var deltaX: number = event.pageX - $(player_selector).offset().left;
    ...
    $(player_selector + ' .volume_button').click(
        function () {
            alert('test')
        }
    );

    $(player_selector + ' .resolution_button').click(
        function () {
            alert('test')
        }
    );

    $(player_selector + ' .comment_button').click(
        function () {
            alert('test')
        }
    );
...
function displayController(value: boolean) {
    if (value) {
...
function displayPlayIcon(value: boolean) {
    if (value) {
...
function displayBufferIcon(value: boolean) {
    if (value) {
...
function updateProgressBar() {
    var play_precent: number = video_content.currentTime / video_content.duration * 100;
...
function formatSecond(seconds: number): string {
    ...
    var haveplayminite_s: string = haveplayminite.toString();
    var haveplaysecond_s: string = haveplaysecond.toString();
...
function updateBufferBar() {
    ...
    var video_duration: number = video_content.duration;
    for (var i: number = 0; i < video_content.seekable.length; i++) {
        var left: number = video_content.seekable.start(i) / video_duration * 100;
        var width: number = (video_content.seekable.end(i) - video_content.seekable.start(i)) / video_duration * 100;
